...
export default function AdminCategories() {
  ...
  const handleSave = async (values) => {
    setSaving(true);
    try {
      if (modal.data) { await api.put(`/categories/${modal.data._id}`, values); toast.success('Updated'); }
      else { await api.post('/categories', values); toast.success('Created'); }
  ...
  const columns = [
    { title: 'Name', dataIndex: 'name', render: (v) => <strong style={{ color: '#fff' }}>{v}</strong> },
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}>
        <h2 style={{ color: '#fff', fontFamily: 'Bebas Neue, sans-serif', fontSize: 28, letterSpacing: 2, margin: 0 }}>CATEGORIES</h2>
        <Button icon={<PlusOutlined />} onClick={() => handleOpen()} style={{ background: '#e63946', border: 'none', color: '#fff', fontWeight: 600 }}>Add Category</Button>
      </div>
      ...
      <Modal open={modal.open} title={<span style={{ color: '#fff' }}>{modal.data ? 'Edit' : 'New'} Category</span>}
